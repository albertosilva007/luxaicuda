...
__global__ void calculateCFD(float* input, float* output, 
        unsigned int width, unsigned int height, float h) {
    // TODO: COMPLETE KERNEL CALCULATION
   
}


__global__ void calculateCFD(float* u, float* u_new, int nx, int ny, float alpha, float dt, float dx, float dy);
